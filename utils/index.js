...
export function getStrapiURL(path) {
    return `${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_STRAPI_API_URL}`}${path}`;
  }
  ...
  export function getStrapiMedia(url) {
    ...
    return `${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_STRAPI_API_URL}`}${url}`;
  }
